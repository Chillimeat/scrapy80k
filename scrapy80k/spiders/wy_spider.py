...
import pymongo
import scrapy
from scrapy80k.items import Scrapy80KItem
from scrapy.conf import settings


class WYSpider(scrapy.Spider):
    name = "wy"
    allowed_domains = ["192.168.88.247"]
    start_urls = [
        'http://192.168.88.247/searchbug.php?q='
    ]
    ...
    def parse(self, response):
        ...
        for n in range(1,end_page + 1):
            page = "/searchbug.php?q=&page=%d" %n
            url = response.urljoin(page)
            yield scrapy.Request(url, self.parse_list)
        # page = response.url.split("/")[-2]
        # filename = 'bugs-page-%s.html' % page
        # with open(filename, 'wb') as f:
        #     f.write(response.body)
        # self.log('Saved file %s' % filename)
    
    def parse_list(self,response):
        links = response.xpath('//div/div/span/p/a/@href').extract()
        for url in links:
            if url.startswith('bug_detail'):
                wooyun_id = url.split('=')[1]
    ...
    def parse_detail(self,response):
        item = Scrapy80KItem()
        item['wooyun_id'] = response.xpath('//*[@id="bugDetail"]/div/h3[1]/a/@href').extract()[0].split('=')[1]
        ...
        dt = response.xpath('//*[@id="bugDetail"]/div/h3[5]/text()').re("[\d+]{4}-[\d+]{2}-[\d+]{2}")[0].split('-')
